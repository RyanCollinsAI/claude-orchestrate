...
(`coursegrid`, `fh-ideas`, `skills-video`, `routines`, `orchestrator`, ...). Podium builds an **All**
...
`context_md` carries the actual artefact, not a description of it. Since 2026-09-04 it renders
under a collapsed **Details** toggle on the question card, closed by default - the card itself
shows only the title, one line per option, and the pick - so putting the real thing here no longer
costs the human a wall of text; it is one click away instead of always on screen:
...
line and report-file landing through `board.sync_board(new_updates=[...])`, so it shows up on Podium
